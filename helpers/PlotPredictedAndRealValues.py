...
def plot_predicted_and_real_values(date, y_test, y_prediction, plot_title,
                                   plots_directories,  neurons=0, number_of_cities=0):
    ...
    plt.title(title if number_of_cities == 0 else f'{title} for {number_of_cities} cities')
    ...
    return evaluator.willmott_index()
